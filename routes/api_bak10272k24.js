...
router.get('/loginpost/:uid/:pwd',async(req,res)=>{
    ...
    .then((db)=>{
		...
		let sql =`select * from zonked_users where email='${req.params.uid}' and pwd='${req.params.pwd}' and verified=1` 
        console.log(`${sql}`)
        db.query( sql, (err,data) => { 
			...
			if ( data.rows.length == 0) {  //data = array 
			  ...
			  res.write(JSON.stringify({
				...
				message	: 	`Welcome to OVU System, ${data.rows[0].full_name.toUpperCase()}! `,
				voice	: 	`Welcome to OVU System, ${data.rows[0].full_name}! `,		
...
//get patient history
router.get('/getpatienthistory/:docid/:case_no', async (req, res) => {	
    ...
    .then((db)=>{
		...
		if(req.params.case_no!=="0"){
			casetext = ` a.dr_id = ${req.params.docid} and a.case_no='${req.params.case_no}' and a.status = 0 `
		}else{
			casetext = ` a.dr_id = ${req.params.docid} and a.status = 0 `
		}//eif
	
		let sql = `select a.*,b.email,c.license
			...
			if ( data.rows.length == 0) {   //data = array 
				...
				`
					...
					for(let zkey in data.rows){
						...
						cnt++
						xtable+= `<tr>
						<td ><i class="fa fa-ambulance"></i> <b>Case No.</b></td>
				...
				xtable+=	
					...
					<input type='text' readonly id='patient_emailx' name='patient_emailx' value='${patientemail}' >
					<input type='text' readonly id='doc_license' name='doc_license' value='${doclicense}' >
					
					`
				//console.log( '***patient history ', data.rows )
				closePg(db);//CLOSE connection
				//cookie
				res.status(200).send(xtable)
    ...
    .then((db)=>{
		...
		let sql = `select
				* from zonked_doctor_schedule 
				where user_id = ${req.params.userid} and status = 0 and book_year = date_part('year',current_date) 
			...
			if ( data.rows.length == 0) {   //data = array 
				...
				`
					...
					for(let zkey in data.rows){
						...
						<button type='button' disabled id='call-btn-${zkey}' class='btn btn-success' 
...
router.get('/getdoctorsched/:doc_id/:doc_name', async(req,res) => {
	...
	.then((db)=>{
		...
		db.query(sql,(err,data)=>{
			console.log('mydata getdoctorSched() ',data.rows)
						...
						`<div class="row row-striped">
							...
							<div class="col-10">
								...
								<ul class="list-inline text-nowrap">
									...
									for(let mkey in data.rows){
										if(data.rows[mkey].book_day == xx){
											//adontMain.push( xx)
											//aDont.push( data.rows[mkey].book_hour)
											if( _xhour.includes( data.rows[mkey].book_hour) ){
												
												let pos = _xhour.indexOf( data.rows[mkey].book_hour )
